...
namespace ProjectMonitors.Monitor.App.Sites.PsDirect
{
  ...
  public class PsDirectFetcherFactory : ProductStatusFetcherFactoryBase
  {
    ...
    public override Result<string> ParseRawTargetInput(string raw)
    {
      ...
      {
        var match = SkuRegex.Match(uri.Segments[4].Split(".")[1]);
        if (match.Success && uri.Host.Contains("direct.playstation"))
        {
    ...
    public override async ValueTask<WatchTarget> CreateTargetAsync(string raw, CancellationToken ct = default)
    {
      ...
      //todo: figure out how to find the product link
    ...
    {
      return new PsDirectFetcher(_monitorHttpClientFactory.CreateHttpClient(), target.Input, _jsonSerializer);
    }
  }
}